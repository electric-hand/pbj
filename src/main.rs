...
fn main() {
    ...
    let prefix_separator = config.prefix_separator.unwrap_or(DEFAULT_PREFIX_SEPARATOR.to_string());
    let variant = args.variant.unwrap_or(config.variant.unwrap_or(DEFAULT_VARIANT.to_string()));

    let prefix = match prefix.as_str() {
        "" => "",
        _ => {prefix.push_str(&prefix_separator); &prefix}
    };
